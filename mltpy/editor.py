...
class MLTEditor:
    ...
    def add_video_clip(self, file_path: Union[str, Path], speed: float = 1.0, 
        ...
        Args:
            file_path: 動画ファイルのパス
            speed: 再生速度倍率
            extra_properties: 追加プロパティ
            
        Raises:
            MediaFileNotFoundError: 動画ファイルが見つからない場合
            InvalidDurationError: 無効な速度が指定された場合
        """
        file_path = Path(file_path)
        if not file_path.exists():
            raise MediaFileNotFoundError(file_path)
        
        if speed <= 0:
            raise InvalidDurationError(f"無効な速度: {speed}")
        
        # 動画の長さを取得
        duration = MediaUtils.get_video_duration(file_path, speed)
        
        # timewarpサービスを使用（速度調整のため）
        mlt_service = "timewarp" if speed != 1.0 else "avformat"
        
        self._add_producer_with_entry(
            file_path, duration, mlt_service, extra_properties
        )
    
    def add_image_clip(self, file_path: Union[str, Path], duration: str = "00:00:05.000",
                      extra_properties: Optional[Dict[str, str]] = None):
        """
        画像クリップを追加
        ...
        Raises:
            MediaFileNotFoundError: 画像ファイルが見つからない場合
            InvalidDurationError: 無効な時間形式の場合
        """
        file_path = Path(file_path)
        if not file_path.exists():
            raise MediaFileNotFoundError(file_path)
        
        if not MediaUtils.validate_duration_format(duration):
            raise InvalidDurationError(duration)
        
        self._add_producer_with_entry(
            file_path, duration, "qimage", extra_properties
        )
    
    def add_text_overlay(self, text: str, duration: str = "00:01:00.000",
        ...
        producer_elem, next_filter_id = self._create_text_producer(
            self.producer_id_counter, self._get_max_id('filter') + 1, 
